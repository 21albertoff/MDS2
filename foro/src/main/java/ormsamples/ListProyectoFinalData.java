...
public class ListProyectoFinalData {
	...
	public void listTestData() throws PersistentException {
		...
		int length = Math.min(commdsforoMensajeDBs.length, ROW_COUNT);
		...
		length = Math.min(commdsforoUsuario_DBs.length, ROW_COUNT);
		for (int i = 0; i < length; i++) {
	...
	public void listByCriteria() throws PersistentException {
		...
		int length =commdsforoMensajeDBs== null ? 0 : Math.min(commdsforoMensajeDBs.length, ROW_COUNT); 
		for (int i = 0; i < length; i++) {
			 System.out.println(commdsforoMensajeDBs[i]);
		}
		...
		length =commdsforoUsuario_DBs== null ? 0 : Math.min(commdsforoUsuario_DBs.length, ROW_COUNT); 
		for (int i = 0; i < length; i++) {
			 System.out.println(commdsforoUsuario_DBs[i]);
		}
